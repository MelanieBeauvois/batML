...
def run_classifier(model, audio, file_dur, samp_rate, detection_thresh, chunk_size):
    """
    ...
    detection_thresh : float
        Threshold above which the confidence level needs to be to consider the prediction as a call.
    chunk_size : float
        Size of an audio chunk.
    ...
    for chunk_id, st_position in enumerate(st_positions):
        ...
        if pos.shape[0] > 0:
            ...
            if chunk_id == (len(st_positions)-1):
                ...
                inds = (prob >= detection_thresh) & (pos < (chunk_size-(model.params.window_size/2.0)))
...
if __name__ == "__main__":
    ...
    detection_thresh = 0.0   # Threshold between 0 and 1. Make this bigger if you want less calls detected
    ...
    model_name = "batmen" # one of: 'batmen', 'cnn2',  'hybrid_cnn_svm',
    ...
    for file_cnt, file_name in enumerate(audio_files):
        ...
        if file_dur>4:
            # run classifier
            tic = time.time()
            call_time, call_prob, call_classes = run_classifier(model_cls, audio, file_dur, samp_rate, detection_thresh, chunk_size)
            toc = time.time()
