...
class Classifier:
    ...
    def train(self, files, gt_pos, durations, gt_classes, test_files, test_pos, test_durations, test_classes):
        '''
        Takes the file names and ground truth call positions and trains the model.
        ...
        test_pos : ndarray
            Ground truth position of the calls for each test file.
        ...
        if self.params.num_hard_negative_mining > 0:
            ...
            for hn in range(self.params.num_hard_negative_mining):
                ...
                save_model(self.params.classification_model, self, self.params.model_dir)

                # evaluate on the test files
                ...
                with open(self.params.model_dir + self.params.model_identifier_classif + '_perf_params.txt', 'a') as f:
                    f.write('time hnm'+str(hn)+' = '+str(toc_global-tic_global))
    ...
    def test_batch(self, goal, files, durations):
        ...
        for ii, file_name in enumerate(files):
            nms_pos[ii], nms_prob[ii], pred_classes[ii], nb_windows[ii] = self.model.test(goal,
                                                            file_name=file_name, file_duration=durations[ii])
...
def generate_training_positions(files, gt_pos, gt_classes, durations, params, add_neg, goal="classification"):
    ...
    for ii in range(len(files)):
        ...
        if add_neg:
            ...
            positions[ii], class_labels[ii] = extract_train_position_from_file_without_neg_positions(gt_pos[ii], gt_classes_ii, params)
...
def extract_train_position_from_file_without_neg_positions(gt_pos, gt_classes, params):
    """
